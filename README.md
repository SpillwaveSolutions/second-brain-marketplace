...
After this catalog (`0.4.14`) the foundation **Available** pins are:

| Pack | Pin |
|------|-----|
| `second-brain-core` | **0.3.8** |
| `okf-graph-eng` | **0.8.2** |
| `project-knowledge-capture` | **0.9.6** |
| `system-architecture-capture` | **0.5.7** |
| `data-engineering-knowledge-capture` | **0.5.3** |
| `research-knowledge-capture` | **0.2.9** |
...
After the marketplace refresh, open the installed `okf-time-series` `plugin.json` and confirm it shows **0.3.3**. 0.3.1 is the killed emit-schema path.
...
| `project-knowledge-capture` | [project-knowledge-capture](https://github.com/SpillwaveSolutions/project-knowledge-capture) | **0.9.6** |
...
| `research-knowledge-capture` | [research-knowledge-capture](https://github.com/SpillwaveSolutions/research-knowledge-capture) | **0.2.9** |
...
OTS is snapshot-first telemetry (install is not capture). **0.3.3** adds dogfood gates: sample fix (no `.telemetry.md`; hour rollover keeps the same slug), `status`/`check` print walked `.okf-history` paths on `not_opted_in`, `ots smoke`, and setup fail-closed on non-pin models. Open `plugin.json` and confirm **0.3.3** (0.3.1 is the killed emit-schema path). Pointers is the join plane: `Link` is a sibling of `TypedEdge`. Pointers stays **0.2.0**. `okf-remote` is not in this catalog.
